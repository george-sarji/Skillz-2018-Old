        ...
        public static void BuildDefensiveBunker()
        {
            ...
            foreach(var capsule in game.GetEnemyCapsules().Where(cap => cap.Holder!=null))
            {
                ...
                if(closestMothership!=null)
                {
                    ...
                    {
                        closestMothershipThroughWormhole = enemyMotherships.OrderBy(mothership => GameExtension.DistanceThroughWormhole(capsule.Location, mothership.Location, closestWormholeToCapsule, closestWormholeToCapsule.Partner.Location, game.GetAllWormholes())).FirstOrDefault();
